    ...
    >
      ...
      <div 
        className={`absolute top-0 left-0 w-full h-full pointer-events-none z-10 transition-all duration-500 ${
          isRecruiter ? 'recruiter-grid-overlay' : 'candidate-grid-overlay'
        }`} 
      />

      {/* LEFT SECTION: Brand and Role Animations (Hidden on mobile/tablet) */}
      <div 
        className={`hidden md:flex flex-col justify-between p-10 relative z-20 overflow-hidden h-screen max-h-screen transition-all duration-500 ease-in-out shrink-0 grow-0 ${
          isRecruiter ? 'basis-[65%]' : 'basis-3/5'
        }`}
      >
        {/* Brand Header */}
        <div className="flex items-center gap-3 max-w-[220px]">
          <img 
            ...
            className={`h-12 w-auto transition-colors duration-500 ${
              isRecruiter ? 'text-recruiter-text-main' : 'text-candidate-text-main'
            }`} 
          />
        </div>
        ...
        {/* Desktop Footer */}
        <div 
          className={`flex justify-between items-center text-[0.85rem] transition-colors duration-500 ${
          ...
          <div className="flex gap-6">
            <a 
              ...
              className={`no-underline font-medium transition-colors duration-200 ${
                isRecruiter ? 'text-recruiter-text-sub hover:text-recruiter-accent' : 'text-candidate-text-sub hover:text-candidate-accent'
              }`}
            >
              Terms of Service
            </a>
      ...
      <div 
        className={`flex items-center justify-center p-6 md:p-10 relative z-20 h-screen max-h-screen w-full transition-all duration-500 ease-in-out shrink-0 grow-0 ${
          isRecruiter 

...
const RecruiterDashboard = ({ onLogout }) => {
  ...
  const stats = [
    ...
    { label: 'Active Assessments', value: '18', change: '+2 new today', icon: Briefcase },
  ...
  return (
    <div className="bg-dash-light-blue-bg text-dash-dark-purple min-h-screen relative overflow-hidden font-inter flex w-full">
          ...
          >
            ...
            <div className="border-t border-dash-border-gray/25 pt-4 space-y-3">
              <div className="flex items-center gap-3 px-2">
                ...
                <div>
                  <h4 className="text-xs font-semibold text-dash-white-card">Ranjith Kumar</h4>
                  <span className="text-[10px] text-dash-light-purple">Lead Recruiter</span>
                </div>
              </div>
              <button
                onClick={() => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-plus-jakarta font-extrabold tracking-tight text-dash-dark-purple flex items-center gap-2">
                Recruiter Workspace
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-dash-primary-purple/10 border border-dash-border-gray text-dash-primary-purple font-outfit">v1.2</span>
              </h2>
              <p className="text-xs sm:text-sm text-dash-light-purple font-semibold mt-0.5">
                Analyze candidate assessments and coordinate evaluation flows.
              </p>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-dash-border-gray">
                <AnimatePresence mode="popLayout">
                  {filteredCandidates.map((candidate) => {
                    // Pill status styling matching rules:
                    // Success (Completed) -> Success Green (#149470)
                    // Recommended (Under Review) -> Light Green (#7CB08D)
                    // Warning (In Progress / Failed) -> Accent Brown (#84492D)
                    let statusColor = '';
                    let statusBg = '';
      ...
      {/* 6. CREATE ASSESSMENT MODAL (Light Themed) */}
      <AnimatePresence>
        {isCreateModalOpen && (
          <>
            {/* Backdrop overlay */}
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] bg-dash-white-card border border-dash-border-gray rounded-3xl shadow-2xl p-6 z-50 max-sm:w-[92%]"
            >
              <div className="flex items-center justify-between pb-3.5 border-b border-dash-border-gray mb-5">
                <h3 className="text-base font-extrabold text-dash-dark-purple font-outfit">Create Assessment</h3>
                <button
                  onClick={() => setIsCreateModalOpen(false)}
                  className="p-1 rounded-lg hover:bg-dash-soft-pink text-dash-light-purple hover:text-dash-dark-purple transition-all duration-200 cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateAssessmentSubmit} className="space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. React Developer Evaluation"
                    value={newAssessment.title}
                    onChange={(e) => setNewAssessment({ ...newAssessment, title: e.target.value })}
                    className="w-full bg-dash-white-card border border-dash-border-gray rounded-xl py-2.5 px-4 text-xs font-semibold text-dash-dark-purple placeholder-dash-light-purple/60 focus:outline-none focus:border-dash-primary-purple"
                  />
                </div>

                {/* Target Role Selector */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-dash-light-purple uppercase tracking-wide">Target Role</label>
                  <select
                    value={newAssessment.role}
                    onChange={(e) => setNewAssessment({ ...newAssessment, role: e.target.value })}
                    className="w-full bg-dash-white-card border border-dash-border-gray rounded-xl py-2.5 px-4 text-xs font-semibold text-dash-dark-purple focus:outline-none focus:border-dash-primary-purple cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  {/* Duration */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-dash-light-purple uppercase tracking-wide">Duration (mins)</label>
                    <input
                      type="number"
                      required
                      min="10"
                      ...
                      className="w-full bg-dash-white-card border border-dash-border-gray rounded-xl py-2.5 px-4 text-xs font-semibold text-dash-dark-purple focus:outline-none focus:border-dash-primary-purple"
                    />
                  </div>

                  {/* Difficulty */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-dash-light-purple uppercase tracking-wide">Difficulty</label>
                    <select
                ...
                {/* Submitter */}
                <div className="pt-4 border-t border-dash-border-gray flex gap-3 mt-5">
                  <button
                    type="button"
                    onClick={() => setIsCreateModalOpen(false)}
                    className="flex-1 py-2.5 rounded-xl border border-dash-border-gray text-dash-dark-purple bg-dash-white-card hover:bg-dash-soft-pink transition-all duration-200 cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2.5 rounded-xl bg-dash-primary-purple text-dash-white-card font-bold text-xs hover:bg-dash-dark-purple shadow-[0_4px_12px_rgba(87,82,170,0.2)] transition-all duration-200 cursor-pointer border-none"
                  >
                    Create
                  </button>
                </div>
